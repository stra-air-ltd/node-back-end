...
const userLoginPlugin: Hapi.Plugin<undefined> = {
  ...
  register: async (server: Hapi.Server) => {
     ...
     * @returns boolean
     */
    async function userLoginWayPssswordOrId(
      userId: string,
      userPassword: string
    ): Promise<boolean> {
      try {
        const userPasswordRAS = await rsaEncrypt(userPassword);
        const redisResult = await server.methods.redisQuery(
          `GET user_id_password_${userId}`
        );
      ...
      async (
        ...
        switch (requestUserLoginWay) {
          case "userNameOrPassword":
            if (
              await userLoginWayPssswordOrUserName(
                requestUserInput,
                requestUserPassword
              )
            ) {
              request = {
                ...
                data: {
                  token: await server.methods.obtainUserToken(
                    getUserId("userName", requestUserInput)
                  ),
                },
              };
            } else {
          ...
          case "userMailOrPassword":
            if (
              await userLoginWayPssswordOrMail(
                requestUserInput,
                requestUserPassword
              )
            ) {
              request = {
                ...
                data: {
                  token: await server.methods.obtainUserToken(
                    getUserId("userMail", requestUserInput)
                  ),
                },
              };
            } else {
          ...
          case "userIdOrPsssword":
            if (
              await userLoginWayPssswordOrId(
                requestUserInput,
                requestUserPassword
              )
            ) {
